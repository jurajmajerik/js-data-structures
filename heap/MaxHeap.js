export default class MaxHeap {
  ...
  heapifyDown() {
    ...
    while (this.hasLeftChild(currIndex)) {
      ...
      let largerChildIdx = this.getLeftChildIndex(currIndex);
      if (
        ...
        largerChildIdx = this.getRightChildIndex(currIndex);
      }

      // Current item is >= larger child, heap property is restored
      if (this.items[currIndex] >= this.items[largerChildIdx]) {
        break;
      }

      this.swap(currIndex, largerChildIdx);
      currIndex = largerChildIdx;
    }
  }
}
